...
import { normalizarCabecalho, montarLinha } from "@/lib/csv";
...
// primeira linha como cabeçalho. `tipos` (opcional): { campo: "numero" |
// "data" } — ver montarLinha em lib/csv.js.
export async function parseXlsxGenerico(arrayBuffer, { mapaCampos, obrigatorios, tipos = {} }) {
  const { default: ExcelJS } = await import("exceljs");
  const workbook = new ExcelJS.Workbook();
  ...
  for (let numeroLinha = 2; numeroLinha <= sheet.rowCount; numeroLinha++) {
    ...
    // Guarda o valor "cru" (cell.value — número ou Date de verdade quando
    // a célula é desse tipo) além do texto formatado, pra montarLinha
    // poder validar número/data com precisão em vez de reinterpretar
    // uma string já formatada pelo Excel.
    const brutos = [];
    linhaExcel.eachCell({ includeEmpty: true }, (cell, colNumber) => {
      const valor = cell.value;
      brutos[colNumber - 1] = valor != null && typeof valor === "object" && !(valor instanceof Date)
        ? String(cell.text || "").trim() // fórmula/rich-text: usa o texto calculado
        : valor instanceof Date || typeof valor === "number"
          ? valor
          : String(cell.text || "").trim();
    });
    if (brutos.every((v) => v === "" || v == null)) continue; // linha em branco — ignora sem avisar

    const { linha, errosLinha } = montarLinha(mapaCampos, obrigatorios, tipos, indicePorCampo, (campo) =>
      indicePorCampo[campo] !== undefined ? brutos[indicePorCampo[campo]] ?? "" : ""
    );
    if (errosLinha.length > 0) {
      erros.push(`Linha ${numeroLinha}: ${errosLinha.join("; ")} — ignorada.`);
      continue;
    }
    linhas.push(linha);
  }

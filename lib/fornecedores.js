...
export async function buscarOuCriarFornecedorGlobal(supabase, documentoBruto, dados) {
  ...
  const { data: existente, error: buscaError } = await supabase
    ...
    .select("id, razao_social, nome_fantasia, endereco, categoria, categorias")
    .eq("cnpj", cnpj)
    .maybeSingle();
  if (buscaError) return { erro: buscaError.message };
  if (existente) {
    // Bug real corrigido aqui: antes, ao reaproveitar um fornecedor global
    // já existente pelo CNPJ, a função só devolvia o id — nunca
    // atualizava categoria/categorias. Se o primeiro cadastro desse CNPJ
    // na base (feito por qualquer condomínio) tivesse ficado sem
    // categoria, o registro global ficava permanentemente sem categoria
    // — e nenhum outro condomínio conseguia mais achá-lo pela busca por
    // categoria da Rede Vizinn (buscar_fornecedores_rede filtra por
    // categoria na tabela global), mesmo que o cadastro local estivesse
    // com a categoria certa. Agora, a cada cadastro/importação que
    // reaproveita o CNPJ, mescla a(s) categoria(s) informada(s) com as
    // que já existiam no global (sem duplicar) e completa nome
    // fantasia/endereço se estiverem vazios — sem nunca sobrescrever
    // dados já preenchidos.
    const categoriasExistentes = existente.categorias?.length
      ? existente.categorias
      : existente.categoria
        ? [existente.categoria]
        : [];
    const categoriasNovas = dados.categorias || [];
    const categoriasMescladas = Array.from(new Set([...categoriasExistentes, ...categoriasNovas]));

    const updates = {};
    if (categoriasMescladas.length && categoriasMescladas.join("|") !== categoriasExistentes.join("|")) {
      updates.categorias = categoriasMescladas;
      updates.categoria = existente.categoria || categoriasMescladas[0] || null;
    }
    if (!existente.nome_fantasia && dados.nomeFantasia) updates.nome_fantasia = dados.nomeFantasia;
    if (!existente.endereco && dados.endereco) updates.endereco = dados.endereco;

    if (Object.keys(updates).length) {
      updates.updated_at = new Date().toISOString();
      await supabase.from("fornecedores_globais").update(updates).eq("id", existente.id);
    }

    return { id: existente.id, jaExistia: true, razaoSocialGlobal: existente.razao_social };
  }

...
// Registros já finalizados não têm indicador (o prazo deixou de importar)
// — statusFinais é customizável porque Manutenção reaproveita esta mesma
// função com seus próprios status finais (["concluida"]).
export function calcularStatusPrazo(dataPrevista, status, statusFinais = STATUS_FINAIS) {
  if (!dataPrevista || statusFinais.includes(status)) return null;

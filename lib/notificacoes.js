...
export async function notificarPessoa(supabaseAdmin, { condominioId, evento, referenciaId, nome, email, whatsapp, template, parametrosEmail, parametrosWhatsapp }) {
  const resultados = [];

  if (email) {
    try {
      await enviarEmail({ to: email, subject: parametrosEmail.subject, html: parametrosEmail.html });
